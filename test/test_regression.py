...
def run_simulator(n, w, signed, acc, initials):
  ...
  cfg, initial_cfg = build_cfg(w, signed, acc, initials)
  ...
  if passed < 1:
    ...
    if initials:
      I = np.append(I, initial_cfg)
      np.savez_compressed(data_file, I)
    else:
...
def build_cfg(w, signed, acc, initials):
  if initials:
    cfg_initials = [randint(0, (2**32)-1), randint(0, (2**32)-1), randint(0, (2**32)-1), randint(0, (2**32)-1)]
  else:
    cfg_initials = [0, 0, 0, 0]
  
  return int(log(w/8)/log(2)) + ((1 if acc else 0) << 2) + ((1 if signed else 0) << 3), cfg_initials
